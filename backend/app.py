...
async def add_review(review: ReviewRequest):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM product WHERE name = %s", (review.item_name,))
        ...
        cursor.execute(
            ...
            VALUES (%s, %s, %s, %s)
            """,
            (user_id, product_id, review.rating, review.review),
        )
        conn.commit()
...
@app.get("/product", response_model=list[ProductItem])
def get_product():
    conn = get_connection()
    try:
        cursor = conn.cursor()
...
async def search_product(query: str = Query(..., description="Строка для поиска продуктов")):
    conn = get_connection()
    try:
        cursor = conn.cursor()
