...
def display_product():
    st.title("Ассортимент табачной продукции")
    search_product()
    try:
        ...
        if response.status_code == 200:
            ...
            if product_items:
                ...
                st.info("Ассортимент пуст.")
        else:
            st.error(f"Ошибка загрузки ассортимента: {response.status_code}")
...
def search_product():
    st.title("Поиск товара")
    query = st.text_input("Введите строку для поиска")
    if st.button("Поиск"):
        try:
            response = requests.get(f"{API_URL}/product/search", params={"query": query})
            if response.status_code == 200:
...
if "token" not in st.session_state:
    ...
    if user_role == "admin":
        page = st.sidebar.radio("Страницы", ["Ассортимент", "Корзина", "Адрес", "Оформить заказ", "Оплата заказа", "Администраторская"])
    else:
        page = st.sidebar.radio("Страницы", ["Ассортимент", "Корзина", "Адрес", "Оформить заказ", "Оплата заказа"])
