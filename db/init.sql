...
-- Создание таблицы с ассортиментом
...
VALUES
    ('Сигарета чапа', 'Классная сигаретка хочется курить снова и снова', 8.99, TRUE),
    ('Ашкудишка', 'Та самая ашкудишка с твоим любимым вкусом', 5.49, TRUE),
    ('Снюс', 'Ну это чисто для быдла', 7.99, TRUE),
    ('Бонг', '?????', 4.49, TRUE)
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    product_id INT NOT NULL REFERENCES product(id) ON DELETE CASCADE, -- Автоудаление заказов при удалении товара
...
CREATE TABLE IF NOT EXISTS cart (
    ...
    product_id INT NOT NULL REFERENCES product(id) ON DELETE CASCADE, -- Автоудаление элементов корзины при удалении товара
    quantity INT NOT NULL CHECK (quantity > 0), -- Ограничение: количество должно быть положительным
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS reviews (
    ...
    product_id INT NOT NULL REFERENCES product(id) ON DELETE CASCADE, -- Удаление отзывов при удалении товара
...
CREATE TRIGGER log_product_event
